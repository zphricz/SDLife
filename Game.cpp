...
Game::Game(int num_x, int num_y, PerfSoftScreen *screen)
    : num_cells_x(num_x), num_cells_y(num_y), buff_width(num_x + 2),
      buff_height(num_y + 2), buffer_1(new bool[buff_width * buff_height]),
...
bool &Game::cell_at(int x, int y) { return current_state[y * buff_width + x]; }
...
void Game::handle_input() {
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_MOUSEMOTION: {
            static int start_x =
                -1; // These need to be initialized to -1 so that
            static int start_y =
                -1; // first time this case is executed, they can
